...
class ImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=6,
                 sort_by_prob=False, load_in_tmp=True) -> object:
...
#precomp_data (n,512) n为目标数
        delta = self.max_detections - precomp_data.shape[0]
        if delta > 0:
            precomp_data = np.concatenate([precomp_data, np.zeros((delta, precomp_data.shape[1]))], axis=0)  #补0
        elif delta < 0:
            precomp_data = precomp_data[:self.max_detections]

        return precomp_data.astype(np.float32)   #（6,512）



class MatrixField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=6,
                 sort_by_prob=False, load_in_tmp=True) -> object:
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(MatrixField, self).__init__(preprocessing, postprocessing)

    def preprocess(self, x, avoid_precomp=False):
        try:
            precomp_matrix = np.load(os.path.join(self.detections_path, x))

            if self.sort_by_prob:
                precomp_matrix = precomp_matrix[np.argsort(np.max(f['%d_cls_prob' % image_id][()], -1))[::-1]]
        except KeyError:
            warnings.warn('Could not find detections for %d' % image_id)
            precomp_matrix = np.random.rand(10,10)
#precomp_data (n,512) n为目标数
        delta = self.max_detections - precomp_matrix.shape[0]
        if delta > 0:
            precomp_matrix = np.concatenate([precomp_matrix, np.zeros((delta, precomp_matrix.shape[1]))], axis=0)  #补0
            precomp_matrix = np.concatenate([precomp_matrix,np.zeros((self.max_detections,delta))],axis=1) #补成6*6  #邻接矩阵

            D = np.sum(precomp_matrix,0)
            num_node = precomp_matrix.shape[0]
            Dn = np.zeros((num_node,num_node))
            for iii in range(num_node):
                if D[iii]> 0:
                   Dn[iii,iii] = D[iii]**(-1)
            precomp_matrix = np.dot(precomp_matrix,Dn)
            # precomp_matrix = precomp_matrix + sp.eye(precomp_matrix.shape[0])  #加上单位矩阵
            # rowsum = np.sum(precomp_matrix)   #计算度
            #
            # num_node = precomp_matrix.shape[0]
            # Dn = np.zeros((num_node, num_node))
           ...
           precomp_matrix = precomp_matrix[:self.max_detections]


        return precomp_matrix.astype(np.float32)   #（10,10）
...
class TextField(RawField):
    ...
    def preprocess(self, x):
        if six.PY2 and isinstance(x, six.string_types) and not isinstance(x, six.text_type):
            x = six.text_type(x, encoding='utf-8')      #x表示标题
        if self.lower:
