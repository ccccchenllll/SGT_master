...
class MeshedDecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        ...
        enc_att1 = self.enc_att(self_att, enc_output[:, 0], enc_output[:, 0], mask_enc_att) * mask_pad  #（10,18,512）
        ...
        alpha1 = torch.sigmoid(self.fc_alpha1(torch.cat([self_att, enc_att1], -1))) #（10,18,512）
...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=512, dropout=.1,
        ...
        self.word_emb = nn.Embedding(vocab_size, d_model, padding_idx=padding_idx)#word_emb:(45,512)
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        seq = torch.arange(1, seq_len + 1).view(1, -1).expand(b_s, -1).to(input.device)  # (b_s, seq_len) seq(1,2,3,^18)
        ...
        out = self.word_emb(input) + self.pos_emb(seq) #(bs,seq_len,512)
