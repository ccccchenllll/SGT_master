...
class PairedDataset(Dataset):
    ...
    def ad_matrix_set(self):
        ad_matrix_list = [e.ad_matrix for e in self.examples]
        ad_matrix_set = unique(ad_matrix_list)
        examples = [Example.fromdict({'ad_matrix': i}) for i in ad_matrix_set]
        dataset = Dataset(examples, {'ad_matrix': self.ad_matrix_field})
        return dataset
    ...
    def ad_matrix_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset(self.examples, fields, key_fields='ad_matrix')
        return dataset
...
class COCO(PairedDataset):
    def __init__(self, image_field, ad_matrix_field, text_field, img_root, ann_root, id_root):
        roots = {}
        roots['train'] = {
            'img': img_root,
            'ad_matrix': img_root,
        ...
        super(COCO, self).__init__(examples, {'image': image_field,'ad_matrix': ad_matrix_field, 'text': text_field})
    ...
    def get_samples(cls, roots, ids_dataset=None):
        ...
        for split in ['train', 'val']:
            ...
            for index in range(int(ids_len)):
                id_path = anns[index]['id_path']
                ad_path = anns[index]['ad_path']
                caption = anns[index]['caption']
            
                example = Example.fromdict({'image': os.path.join(roots[split]['img'], id_path),'ad_matrix': os.path.join(roots[split]['ad_matrix'], ad_path),
                    'text': caption})
