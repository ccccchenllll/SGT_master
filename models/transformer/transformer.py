...
class Transformer(CaptioningModel):
    ...
    def forward(self, images,ad_matrix, seq, *args):
        enc_output, mask_enc = self.encoder(images,ad_matrix)
    ...
    def step(self, t, prev_output, visual,ad_matrix, seq, mode='teacher_forcing', **kwargs):
        it = None
        if mode == 'teacher_forcing':
            raise NotImplementedError
        elif mode == 'feedback':
            if t == 0:
                self.enc_output, self.mask_enc = self.encoder(visual,ad_matrix)
